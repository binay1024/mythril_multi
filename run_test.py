...
for subdir in os.listdir(data_dir):
    ...
    if os.path.isdir(subdir_path):
        ...
        # 构建 sol文件和 bin文件的 路径
        attackdir = "/opt/mythril/mythril_v2/solidity_examples/ccs2023/AttackBridge"
        ...
        if len(bin_files) ==2:
            ...
            if os.path.exists(attack_bridge_path) and os.path.exists(bin1_path) and os.path.exists(bin2_path):
                ...
                command = "python3 myth.py analyze  {}".format(bin1_path[:-3]) + "sol"
                print(command)

                result = subprocess.run(command, shell=True, text=True)
                command = "python3 myth.py analyze  {}".format(bin2_path[:-3]) + "sol"
                print(command)
                
                result = subprocess.run(command, shell=True, text=True)
                output = "/opt/log/51/T2"
                output_path = os.path.join(output, temp)
                command = "python3 myth.py analyze -mc {} {} {} > {}".format(bin2_path,attack_bridge_path,bin1_path, output_path)
                print(command)
                # 执行命令并捕获输出
                time_start = time.time()
                try:
